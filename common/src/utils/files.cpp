...
{
    log<LOG_VERBOSE>(L"Opening file %1% ...") % filename.c_str();
    std::ifstream fileInput;
    fileInput.exceptions (std::ifstream::failbit | std::ifstream::badbit);
    fileInput.open(filename);
    log<LOG_VERBOSE>(L"Reading file to buffer ...");
    std::stringstream buffer;
    buffer << fileInput.rdbuf();
    log<LOG_VERBOSE>(L"Writing buffer to string ...");
    contents = buffer.str();
    log<LOG_VERBOSE>(L"Closing file %1% ...") % filename.c_str();
    fileInput.close();
}